...
### Примечания
- Имитирую различные ответы от API с помощью функции getRandomUrl
- Для удобства ручного тестирования у кнопки #submitButton после получения статуса success или error, я убираю атрибут disabled (если планировалось автоматическое тестирование, то я буду очень опечален)
- Так как в задании не сказано каким методом и на какой endpoint отправлять форму, я использовал отправку GET-формы
